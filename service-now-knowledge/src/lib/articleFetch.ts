...
export interface KbArticleRecord {
	...
	language?: string;
	kb_knowledge_base_id?: string;
	kb_knowledge_base_name?: string;
}
...
interface KmApiArticle {
	id: string;
	number?: string;
	title?: string;
	snippet?: string;
	link?: string;
	fields?: Record<string, KmApiField>;
}

interface KmApiResponse {
	result: {
		meta: {
			start?: number;
			end?: number;
			count?: number;
			language?: string;
			status?: { code: number; error_message?: string };
		};
		articles: KmApiArticle[];
	};
}
...
export async function* fetchKbArticles(
...
): AsyncGenerator<KbArticleRecord, void, unknown> {
	const filterParts: string[] = [];
	const state = options.workflowState?.trim();
	if (state) filterParts.push(`workflow_state=${state}`);
	...
	let offset = 0;
	while (true) {
		const res = await client.get<KmApiResponse>("/api/sn_km_api/knowledge/articles", {
			...baseParams,
		...
		if (batch.length < options.pageSize) return;
		offset += batch.length;
	}
}

function mapArticle(a: KmApiArticle): KbArticleRecord | undefined {
...
function pickFieldContent(fields: Record<string, KmApiField>, ...names: string[]): string {
	for (const name of names) {
		const f = fields[name];
		if (f?.value && f.value.trim()) return f.value;
		if (f?.display_value && f.display_value.trim()) return f.display_value;
	}
	return "";
}

function findContentSource(fields: Record<string, KmApiField>, ...names: string[]): string | undefined {
	for (const name of names) {
		const f = fields[name];
		if (f?.value && f.value.trim()) return `${name}.value`;
		if (f?.display_value && f.display_value.trim()) return `${name}.display_value`;
	}
	return undefined;
}

export function getKnowledgeBaseId(article: KbArticleRecord): string | undefined {
...
export async function fetchArticleBody(client: ServiceNowClient, sysId: string): Promise<ArticleBodyResult> {
	const res = await client.get<any>(`/api/sn_km_api/knowledge/articles/${sysId}`);
	...
	if (fromContent) return { body: fromContent, debug: `content(${kindOf(result.content)})` };

	const fromEmbedded = extractAny(result.embedded_content);
	if (fromEmbedded) return { body: fromEmbedded, debug: `embedded_content(${kindOf(result.embedded_content)})` };

	const fields = (result.fields ?? {}) as Record<string, KmApiField>;
	const fromFields = pickFieldContent(fields, "text", "wiki", "description");
	if (fromFields) return { body: fromFields, debug: "fields.fallback" };
...
function extractStringish(v: any): string {
...
function extractFromBlocks(blocks: any[]): string {
	...
	for (const block of blocks) {
		...
		if (used) {
			parts.push(used);
		} else if (Array.isArray((block as any).children)) {
			const nested = extractFromBlocks((block as any).children);
...
function firstBlockKeys(v: any): string {
	if (!Array.isArray(v) || v.length === 0) return "";
	const first = v[0];
	if (!first || typeof first !== "object") return `t=${typeof first}`;
	return `keys:${Object.keys(first).slice(0, 8).join(",")}`;
}

function peek(v: any): string {
	if (v === null) return "null";
	if (v === undefined) return "undef";
	if (typeof v === "string") return `"${v.slice(0, 40)}"(len=${v.length})`;
	if (typeof v === "number" || typeof v === "boolean") return String(v);
	if (Array.isArray(v)) return `arr[${v.length}]`;
	if (typeof v === "object") return `obj{${Object.keys(v).slice(0, 6).join(",")}}`;
	return typeof v;
}
