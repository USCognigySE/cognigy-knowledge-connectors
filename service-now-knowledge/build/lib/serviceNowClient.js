...
const serviceNowAuth_1 = require("./serviceNowAuth");
const MAX_RETRIES = 5;
class ServiceNowClient {
    constructor(instance, accessToken) {
        const base = (0, serviceNowAuth_1.normaliseInstanceUrl)(instance);
    ...
    async request(config, attempt = 0) {
        try {
            ...
            const bodySnippet = body
                ...
                : "";
            throw new Error(`ServiceNow request failed [${url}] status=${status ?? "n/a"}${bodySnippet}: ${err?.message ?? err}`);
        }
    }
}
