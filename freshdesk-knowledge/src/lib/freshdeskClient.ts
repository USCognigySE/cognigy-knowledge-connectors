...
import { normaliseFreshdeskBaseUrl, buildAuthHeader } from "./freshdeskAuth";
...
export class FreshdeskClient {
	...
	constructor(domain: string, apiKey: string) {
		const baseURL = normaliseFreshdeskBaseUrl(domain);
	...
	private async request<T>(config: AxiosRequestConfig, attempt = 0): Promise<T> {
		try {
			...
			const bodySnippet = body
				...
				: "";
			throw new Error(
				`Freshdesk request failed [${url}] status=${status ?? "n/a"}${bodySnippet}: ${err?.message ?? err}`
			);
		}
	}
}
