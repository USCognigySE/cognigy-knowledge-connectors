...
async function* crawlDrives(client, site, options) {
    const drives = client.paginate(`/sites/${site.id}/drives`);
...
async function* walkFolder(client, listUrl, libraryName, options) {
    for await (const item of client.paginate(listUrl)) {
        ...
        if (!item.file)
            continue;
        const ext = (0, extractors_1.getExtension)(item.name);
        if (!(0, extractors_1.isSupported)(ext, options.allowedExtensions))
            continue;
        if (typeof item.size === "number" && item.size > options.maxFileSizeBytes)
            continue;
        try {
            const buffer = await client.getBuffer(`/drives/${item.parentReference.driveId}/items/${item.id}/content`);
            const text = await (0, extractors_1.extractText)(buffer, ext);
            if (!text.trim())
                continue;
            yield {
                text,
...
async function* crawlPages(client, site) {
    const pages = client.paginate(`/sites/${site.id}/pages/microsoft.graph.sitePage?$expand=canvasLayout`);
    ...
    for (const section of sections) {
        for (const column of section.columns ?? []) {
            for (const webpart of column.webparts ?? []) {
                ...
                if (html)
                    parts.push((0, extractors_1.extractFromHtmlString)(html));
                else if (webpart?.data?.title)
                    parts.push(webpart.data.title);
            }
        }
    }
